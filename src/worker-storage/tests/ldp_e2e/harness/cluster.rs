...
use crate::storage::engine::duckdb::DuckDBQueryEngine;
...
pub struct TestCluster {
    ...
    pub coordinator: Arc<LdpCoordinator>,
...
impl TestWorker {
    ...
    pub fn new(worker_id: String, port: u16) -> Self {
        let query_engine = Arc::new(
            DuckDBQueryEngine::builder()
                .with_memory_limit_mb(1024)
                .build()
                .expect("Failed to create DuckDB query engine"),
        );
...
impl TestCluster {
    ...
    pub async fn execute_query(
        ...
        self.coordinator.execute_query(sql).await.map_err(|e| e.into())
    }
    ...
    pub async fn execute_query_on_worker(
        ...
        worker.query_engine.execute_query(sql, None, None).await.map_err(|e| e.into())
    }
    ...
    pub async fn load_data_to_worker(
        ...
        for batch in &data {
            worker
                .query_engine
                .execute_query(
                    &format!("CREATE TABLE IF NOT EXISTS {} AS SELECT * FROM arrow_batch", table_name),
                    None,
                    None,
                )
                .await?;

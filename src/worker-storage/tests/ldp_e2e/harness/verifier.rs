...
impl TestVerifier {
    ...
    fn concat_batches(batches: &[RecordBatch]) -> Result<RecordBatch, VerificationError> {
        ...
        let columns: Result<Vec<_>, _> = (0..schema.fields().len())
            .map(|i| {
                let col_arrays: Vec<_> = batches.iter().map(|batch| batch.column(i)).collect();
                arrow::compute::concat(&col_arrays)
            })
            .collect();

        let concatenated_columns = columns.map_err(VerificationError::ArrowError)?;
        RecordBatch::try_new(schema, concatenated_columns).map_err(VerificationError::ArrowError)
    }
    ...
    fn convert_to_sorted_rows(
        batch: &RecordBatch,
        preserve_order: bool,
    ) -> Result<Vec<Row>, VerificationError> {
        ...
        let sort_fields = sort_fields.map_err(VerificationError::ArrowError)?;
        let converter = RowConverter::new(sort_fields).map_err(VerificationError::ArrowError)?;
        ...
        if preserve_order {
            ...
            let mut rows_vec: Vec<_> = rows.into_iter().enumerate().collect();
            rows_vec.sort_by(|a, b| a.1.cmp(&b.1));
            Ok(rows_vec.into_iter().map(|(_, row)| row).collect())
        }
    }
    ...
    fn compare_columns_approximately(
        ...
        match (col1.data_type(), col2.data_type()) {
            ...
            _ => {
                ...
                if !arrow::array::equal(col1, col2) {
...
impl From<ArrowError> for VerificationError {
    fn from(error: ArrowError) -> Self {
        VerificationError::ArrowError(error)
    }
}

...
use crate::ldp::executor::metrics::{
    LdpMetricsRegistry, QueryExecutionMetrics, StageExecutionMetrics,
};
...
impl PerformanceOptimizer {
    ...
    pub async fn analyze_query_performance(&self, query_id: &str) {
        ...
        let stage_metrics = self
            .metrics_registry
            .get_query_stage_metrics(query_id)
            .await;
    ...
    async fn analyze_query_level_performance(&self, metrics: &QueryExecutionMetrics) {
        ...
        let memory_utilization = (metrics.peak_memory_bytes as f64) / (1024.0 * 1024.0); // Convert to MB
    ...
    pub async fn apply_optimizations(&self, query_id: &str) -> Vec<AppliedOptimization> {
        ...
        let query_recommendations: Vec<_> = self
            .recommendations
            .iter()
        ...
        for recommendation in query_recommendations {
            for suggestion in recommendation.suggestions {
                match suggestion {
                    OptimizationSuggestion::SlowStageExecution {
                        stage_id,
                        duration_ms,
                        ..
                    } => {
                        ...
                        applied_optimizations.push(AppliedOptimization::IncreasedParallelism {
                            stage_id,
                            reason: format!(
                                "Stage took {}ms, applying parallelism optimization",
                                duration_ms
                            ),
                        });
                    }
                    OptimizationSuggestion::HighStageMemoryUsage {
                        stage_id,
                        memory_mb,
                        ..
                    } => {
                    ...
                    OptimizationSuggestion::LowProcessingRate {
                        stage_id,
                        rows_per_second,
                        ..
                    } => {
    ...
    pub async fn get_performance_summary(&self, query_id: &str) -> PerformanceSummary {
        ...
        let slow_stages: Vec<_> = stage_metrics
            .iter()
            .filter(|m| {
                m.execution_duration.as_millis() > self.config.slow_stage_threshold_ms as u128
            })
            .collect();
...
pub enum OptimizationSuggestion {
    ...
    HighMemoryUsage { memory_mb: f64, threshold_pct: f64 },
}
...
pub enum AppliedOptimization {
    ...
    IncreasedParallelism { stage_id: StageId, reason: String },

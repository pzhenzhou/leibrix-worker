...
impl SkewHandler {
    ...
    pub fn detect_skew(
        ...
        let mut sorted_keys: Vec<(String, u64)> = key_counts.into_iter().collect();
    ...
    fn handle_skewed_partitioning(
        ...
        let mut partitioned_batches =
            vec![RecordBatch::new_empty(batch.schema()); num_partitions as usize];

        // Create a map of top keys for fast lookup
        let top_key_set: std::collections::HashSet<String> = skew_result
            .top_keys
            .iter()
            .map(|(key, _)| key.clone())
            .collect();
        ...
        for row_idx in 0..batch.num_rows() {
            ...
            let partition_id = if top_key_set.contains(&composite_key) {
                ...
                if self.config.enable_round_robin_distribution {
                    ...
                    let top_key_index = top_key_set
                        .iter()
                        .position(|k| k == &composite_key)
                        .unwrap_or(0);
        ...
        match array.data_type() {
            arrow::datatypes::DataType::Int8 => {
                if let Some(arr) = array.as_any().downcast_ref::<Int8Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Int8Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Int16 => {
                if let Some(arr) = array.as_any().downcast_ref::<Int16Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Int16Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Int32 => {
                if let Some(arr) = array.as_any().downcast_ref::<Int32Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Int32Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Int64 => {
                if let Some(arr) = array.as_any().downcast_ref::<Int64Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Int64Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::UInt8 => {
                if let Some(arr) = array.as_any().downcast_ref::<UInt8Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to UInt8Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::UInt16 => {
                if let Some(arr) = array.as_any().downcast_ref::<UInt16Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to UInt16Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::UInt32 => {
                if let Some(arr) = array.as_any().downcast_ref::<UInt32Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to UInt32Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::UInt64 => {
                if let Some(arr) = array.as_any().downcast_ref::<UInt64Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to UInt64Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Float32 => {
                if let Some(arr) = array.as_any().downcast_ref::<Float32Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Float32Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Float64 => {
                if let Some(arr) = array.as_any().downcast_ref::<Float64Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Float64Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Utf8 => {
                if let Some(arr) = array.as_any().downcast_ref::<StringArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to StringArray".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::LargeUtf8 => {
                if let Some(arr) = array.as_any().downcast_ref::<LargeStringArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to LargeStringArray".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Date32 => {
                if let Some(arr) = array.as_any().downcast_ref::<Date32Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Date32Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Date64 => {
                if let Some(arr) = array.as_any().downcast_ref::<Date64Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Date64Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Timestamp(_, _) => {
                ...
                } else if let Some(arr) = array.as_any().downcast_ref::<TimestampMillisecondArray>()
                {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Timestamp array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Decimal128(_, _) => {
                if let Some(arr) = array.as_any().downcast_ref::<Decimal128Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Decimal128Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Decimal256(_, _) => {
                if let Some(arr) = array.as_any().downcast_ref::<Decimal256Array>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to Decimal256Array".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Boolean => {
                if let Some(arr) = array.as_any().downcast_ref::<BooleanArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to BooleanArray".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::Binary => {
                if let Some(arr) = array.as_any().downcast_ref::<BinaryArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to BinaryArray".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::LargeBinary => {
                if let Some(arr) = array.as_any().downcast_ref::<LargeBinaryArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to LargeBinaryArray".to_string(),
                    ))
                }
            }
            arrow::datatypes::DataType::FixedSizeBinary(_) => {
                if let Some(arr) = array.as_any().downcast_ref::<FixedSizeBinaryArray>() {
                    ...
                    Err(ExchangeError::PartitionFailed(
                        "Failed to cast to FixedSizeBinaryArray".to_string(),
                    ))
                }
            }
    ...
    fn concatenate_batches_with_schema(
        ...
        if batch1.schema() != batch2.schema() {
            return Err(ExchangeError::ConcatFailed(
                "Schemas do not match".to_string(),
            ));
        }

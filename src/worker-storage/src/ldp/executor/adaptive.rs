...
impl AdaptivePartitioner {
    ...
    pub fn analyze_distribution(&self, batch: &RecordBatch, field_indices: &[u32]) -> DataDistributionStats {
    ...
    fn analyze_column_distribution(&self, column: &dyn Array, total_rows: u64) -> DataDistributionStats {
        use arrow::array::*;
        use std::collections::HashMap;
        ...
        let skew_factor = calculate_skew_factor(&value_counts.values().cloned().collect::<Vec<_>>());
        
        // Get top values for skew analysis
        let mut top_values: Vec<(String, u64)> = value_counts
            .into_iter()
            .collect();
    ...
    pub fn adaptive_partition(
        ...
        match strategy {
            ...
            PartitionStrategy::SkewAware { num_partitions: partitions, top_values } => {
                ...
                debug!("Applying skew-aware partitioning with top values: {:?}", top_values);
                hash_partition_batch(batch, field_refs, partitions)
            }
        }
    }

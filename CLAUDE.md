...
The Leibrix Distributed Plan (LDP) uses a **single-pass unified algorithm** inspired by Volcano/Cascades optimizers, but simplified with a "Substrait-first" approach.
...
```
For each operator in Substrait tree (bottom-up recursion):
...
- Stages contain Substrait fragments with exchange inputs replaced by placeholders
...
- `ldp/substrait/reconstruct.rs` - Substrait fragment reconstruction
...
Tests in `src/worker-storage/tests/ldp_e2e_*.rs` use `MockCluster` to simulate distributed execution:
- `MockCluster::new(num_workers)` creates in-memory workers
- `load_test_data_for_e2e()` populates workers with partitioned data
...
3. Update Substrait reconstruction in `ldp/substrait/reconstruct.rs` if needed
...
- **Substrait Execution**: Full DuckDB Substrait execution via `from_substrait()` is partially implemented
...
- Substrait execution: `worker-storage/src/engine/duckdb/substrait.rs`
